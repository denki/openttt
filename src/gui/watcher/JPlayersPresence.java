...
public class JPlayersPresence extends Watcher {
	...
	public JPlayersPresence(Main main) {
		...
		table = new JTable(getTableCells(),
				new String[] {Language.get("present"), Language.get("paid"), Language.get("player")});
	...
	public void refresh() {
		refreshList();
		if (!players.isEmpty()) {
			table.setModel(new DefaultTableModel(getTableCells(), 
					new String[] {Language.get("present"), Language.get("paid"), Language.get("player")}));
			TableCellEditor df = new DefaultCellEditor(new JCheckBox(""));
			TableCellRenderer cr = new TableCellRenderer() {
				@Override
	...
	private void filter() {
		if (aFilter != null) {
			...
			for (Player p : players)
				if (!matches(p, filter)) toRemove.add(p);
	...
	private boolean matches(Player p, String filter) {
		for (String exp : filter.toLowerCase().split(" "))
			if (!p.getFullName().toLowerCase().contains(exp)) return false;
		return true;
	}
